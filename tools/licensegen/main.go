...
// Command licensegen issues Ledger Enterprise license keys. It is a separate main
// package guarded by the "licensegen" build tag, so it never ships in the default
// ledger release binary (`go build ./...` and `go build ./cmd/ledger` both skip it).
...
const usage = `licensegen — issue Ledger Enterprise license keys (offline; never ships in the ledger binary)
