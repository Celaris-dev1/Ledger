...
func testLicense(edition string, expires time.Time) *License {
	return &License{
		LicenseID: "lic_test", Customer: "Acme Inc", Product: "ledger",
...
func TestResolveStates(t *testing.T) {
	...
	if !graceSt.HasFeature(FeatureComplianceExport) {
	...
	if expSt.HasFeature(FeatureComplianceExport) {
...
func TestHasFeatureByEdition(t *testing.T) {
	...
	if !ent.HasFeature(FeatureComplianceExport) || !ent.HasFeature(FeatureKMSSigner) || !ent.HasFeature(FeatureMultiTenancy) {
		t.Error("enterprise should have compliance-export, kms-signer and multi-tenancy")
	}

	none := &License{Edition: ""}
	if none.HasFeature(FeatureComplianceExport) {
		t.Error("no edition should not unlock enterprise features")
	}

	custom := &License{Edition: "", Features: []string{FeatureComplianceExport}}
	if !custom.HasFeature(FeatureComplianceExport) {
...
func TestLoadTamperedTokenFallsBackToCommunityWithWarning(t *testing.T) {
	...
	t.Setenv("LEDGER_LICENSE", tampered)
	t.Setenv("LEDGER_LICENSE_FILE", "")
	st := Reload()
	if st.State != StateNone {
...
func TestRequireGating(t *testing.T) {
	...
	t.Setenv("LEDGER_LICENSE_FILE", "")
	Reload()
	if err := Require(FeatureComplianceExport); err == nil {
