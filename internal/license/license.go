// Package license implements Ledger's offline license keys for the paid Enterprise
// add-ons. Everything here works without any network access: a license is a small signed
// token the customer sets via LEDGER_LICENSE or LEDGER_LICENSE_FILE, verified against
// vendor public keys embedded in the binary. There is no phone-home, no activation server
// and no telemetry. Ledger's core (append/verify/replay/anchor/incident response) never requires a license at all
// -- see PRICING.md.
package license
...
// Edition names. Ledger has a single paid edition today: Enterprise.
const (
...
const (
	// FeatureComplianceExport gates signed compliance evidence export (EU AI Act /
	// SOC 2 / HIPAA auditor packs: `ledger export --regime ...`). Enterprise.
	FeatureComplianceExport = "compliance-export"
	// FeatureKMSSigner gates external KMS/Vault root signing (LEDGER_SIGNER=vault|awskms).
	// Enterprise.
...
type License struct {
	...
	Product   string    `json:"product"` // always "ledger"
...
const (
	...
	// StateValid: a signed, unexpired license for product "ledger". Gated features work.
...
// Current returns the process-wide license status, loading it from LEDGER_LICENSE /
// LEDGER_LICENSE_FILE on first use and caching it thereafter. Call Reload to re-read it
...
// Reload re-reads LEDGER_LICENSE / LEDGER_LICENSE_FILE and replaces the cached status. It
...
func load() *Status {
	tok := strings.TrimSpace(os.Getenv("LEDGER_LICENSE"))
	if tok == "" {
		if fp := strings.TrimSpace(os.Getenv("LEDGER_LICENSE_FILE")); fp != "" {
			b, err := os.ReadFile(fp)
			if err != nil {
				return &Status{State: StateNone, Warning: fmt.Sprintf("license: could not read LEDGER_LICENSE_FILE (%v); running as community", err)}
			}
...
func Require(feature string) error {
	...
	switch st.State {
	case StateExpired:
		msg = fmt.Sprintf("%q requires a Ledger %s license (yours expired on %s, past the %d-day grace period). Contact sales (see PRICING.md), then set LEDGER_LICENSE / LEDGER_LICENSE_FILE.",
			feature, strings.Title(need), st.License.ExpiresAt.Format("2006-01-02"), int(Grace.Hours()/24))
	case StateValid, StateGrace:
		msg = fmt.Sprintf("%q requires a Ledger %s license; your current license is %s edition. See PRICING.md.", feature, strings.Title(need), st.License.Edition)
	default: // StateNone
		msg = fmt.Sprintf("%q is a Ledger %s feature. Ledger's core stays free; contact sales for Enterprise, then set LEDGER_LICENSE or LEDGER_LICENSE_FILE. See PRICING.md.", feature, strings.Title(need))
	}
...
// warning if active exceeds seats. It is advisory only: Ledger never locks anyone out
...
func Verify(token string) (*License, error) {
	...
	if lic.Product != "ledger" {
		return nil, fmt.Errorf("license is for product %q, not ledger", lic.Product)
	}
