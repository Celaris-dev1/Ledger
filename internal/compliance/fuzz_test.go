...
func FuzzExtractFromPDF(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, pos uint32, val byte, ins []byte, mode uint8) {
		...
		if !bytes.Equal(b, fuzzPDF) {
			t.Fatal("edited PDF verified (pages must match the signed report's rendering)")
		}
	})
}
...
func TestExtractFromPDFHostileLength(t *testing.T) {
	...
	if r, err := VerifyPDF(edited, fuzzTrust); err == nil || !strings.EqualFold(r.DocumentHash, fuzzReport.DocumentHash) {
		t.Fatalf("page edit must be refused: err=%v", err)
	}
	if _, err := VerifyPDF(fuzzPDF, fuzzTrust); err != nil {
		t.Fatalf("genuine PDF: %v", err)
	}
}
