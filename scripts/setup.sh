#!/usr/bin/env bash
# One-shot local setup for Ledger: checks prerequisites, builds, prepares Postgres
...
#   scripts/setup.sh --no-db    build only (ledgerd itself needs Postgres)
set -euo pipefail
cd "$(dirname "$0")/.."
...
mkdir -p bin
go build -o bin/ ./cmd/ledger ./cmd/ledgerd
ls bin
...
case "$MODE" in
  ...
  local)
    if command -v psql >/dev/null && psql "$PG_ADMIN_URL" -tAc 'select 1' >/dev/null 2>&1; then
      ...
      warn "Ledger needs Postgres to run; set LEDGER_DATABASE_URL in .env before starting"
    fi
    ;;
  none) warn "Ledger needs Postgres to run; set LEDGER_DATABASE_URL in .env before starting" ;;
esac
...
if [ -f .env ]; then
  ...
  umask 077
  {
    echo "LEDGER_DATABASE_URL=${DB_URL}?sslmode=disable"
    echo "LEDGER_ADDR=:8410"
    echo "# every /v1 call needs Authorization: Bearer <token>"
    echo "LEDGER_TOKEN=$(rand_hex 24)"
    echo "# Ed25519 seed used to sign anchors"
    echo "LEDGER_SIGNING_KEY=$(rand_b64 32)"
  } > .env
fi
...
Next steps:
  set -a; . ./.env; set +a
  bin/ledgerd &                                             # API on :8410
  curl -s -H "Authorization: Bearer \$LEDGER_TOKEN" localhost:8410/v1/chains/gate/verify
  bin/ledger verify                                         # verify every chain
