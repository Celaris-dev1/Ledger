...
It also verifies every `stack-receipt/v1` envelope it finds at `payload.receipt` on those records
against the enrolled receipt-key trust keyring by default (see "Receipt key enrollment" below,
falling back to `LEDGER_KEYRING_DIR` for a key id the trust keyring doesn't know) — a signed
cross-product envelope (see `docs/receipt-spec.md`) that Warrant, Harbour, Gate, Proof and Bench
can emit at the same points they already call into Ledger — and reports per-receipt `trusted`
status (enrolled + unrevoked as of the receipt's `issued_at` → true; revoked before that → false;
unknown key → false) plus whether each receipt's `links` resolve to other records in the same
incident.

**stack-receipt/v1:** a unified signed envelope (`internal/receipt`, spec in
`docs/receipt-spec.md`) any product signs to attest to a decision/effect/verification, linking to
other products' records. `ledger verify-receipt [--file PATH] --pubkey BASE64 --alg ed25519`
verifies one offline, independent of any Ledger instance; without `--pubkey`, it looks the
envelope's `signer_key_id` up in the enrolled receipt-key keyring instead (needs
`LEDGER_DATABASE_URL`; pass `--no-db` to force the fully offline, no-trust-decision path).
Conformance vectors (valid + invalid fixtures, regenerated by `go test
./internal/receipt/... -run TestConformanceVectors`) live under `testdata/receipts/` for every
other product to copy and validate their own emitters against.

**Receipt key enrollment (`internal/receiptkeys`):** the trust keyring behind the `trusted` field
above is a Postgres table (`receipt_keys`, migration `0700_receipt_keys.sql`), populated
explicitly per product key — nothing is trusted until an operator enrolls it:

```
ledger keys enroll --product gate --key-id ed25519:181264368368b1b0 --public-key <base64|@file.pub> [--alg ed25519|ecdsa-p256-sha256]
ledger keys list [--product gate]      # also lists the root-signing keyring, if LEDGER_KEYRING_DIR is set
ledger keys revoke --key-id ed25519:181264368368b1b0 --reason "rotated"
```

The key id is the one `receipt.Sign`/product signers already compute
(`ed25519:`+hex(sha256(pubkey)[:8]), see `keys.KeyIDFor`), so an enrolled key matches what a
receipt actually gets signed with. Revoking a key does not retroactively untrust receipts it
signed before the revocation: `ledger incident`/`verify-receipt` compare the key's `revoked_at`
against each receipt's own `issued_at`. The same operations are available over HTTP, operator-only
(`PermAdmin`, same tier as API token management): `POST /v1/receipt-keys` (enroll), `GET
/v1/receipt-keys?product=` (list), `POST /v1/receipt-keys/{key_id}/revoke`.

**Stack demo:** `scripts/stack-demo.sh` builds real binaries (`go build`, never `go run`) of
Ledger, Warrant, Harbour, Gate and Proof from sibling checkouts, runs one small real goal across
them, and asserts the resulting incident is complete and every chain verifies — writing
`stack-demo-out/incident.{json,html,md}`.
...
Ledger is open core. Everything above — append/verify/replay/external anchoring, incident
review, projections, single-tenant `ledgerd`, retention/holds/erasure, backup/restore — is
free and always will be; see [PRICING.md](PRICING.md). A small set of *Enterprise* add-ons
require a paid, offline license key: `LEDGER_LICENSE` (a token) or `LEDGER_LICENSE_FILE` (a
path). Verification is Ed25519-signature checking against a vendor public key compiled into
the binary (`internal/license`) — no network call, no phone-home, works air-gapped.

Gated features:

- **Compliance evidence exports** — `ledger export --regime eu-ai-act|soc2|hipaa` (the
  signed auditor pack). `ledger export --verify` (checking an existing pack) stays free.
- **External KMS/Vault root signing** — `LEDGER_SIGNER=vault|awskms`. The default file-based
...
with a warning — Ledger never fails a build over a bad token, and never deletes or hides
data when a feature turns off (`ledgerd` refuses to start with a gated config instead).

- `ledger license show [--json]` / `ledger license verify <token|file>` — inspect the
...
`go build ./...` and the released `ledger`/`ledgerd` binaries never include it. After
generating a real keypair, put its **public** key into `internal/license/keys.go`'s
`prodPublicKeyB64` and cut a release; the private key stays with the vendor, offline,
always.

The repo also carries a public **dev** keypair (`licensegen issue --key dev`) for tests.
Release builds never trust it; only binaries built with `-tags licensedev` (the e2e
harness) do. Never ship a `licensedev` build.
