...
against the configured keyring (`LEDGER_KEYRING_DIR`) — a signed cross-product envelope (see
`docs/receipt-spec.md`) that Warrant, Harbour, Gate, Proof and Bench can emit at the same points
they already call into Ledger — and reports per-receipt signature status plus whether each
receipt's `links` resolve to other records in the same incident.
...
verifies one offline, independent of any Ledger instance. Conformance vectors (valid + invalid
fixtures, regenerated by `go test ./internal/receipt/... -run TestConformanceVectors`) live under
`testdata/receipts/` for every other product to copy and validate their own emitters against.
