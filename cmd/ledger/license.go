...
const licenseUsage = `ledger license show            — current license state, customer, edition, features, seats, expiry
ledger license verify <token|file>
                              — verify a license token (or a file holding one) offline and print it

env: LEDGER_LICENSE (token) or LEDGER_LICENSE_FILE (path), read by every ledger command that
...
func cmdLicense(args []string) error {
	...
	case "verify":
		if len(args) < 2 {
			return fmt.Errorf("usage: ledger license verify <token|file>")
		}
	...
	default:
		fmt.Print(licenseUsage)
		return fmt.Errorf("ledger license: unknown subcommand %q", args[0])
	}
}
...
func cmdLicenseShow(asJSON bool) error {
	...
	fmt.Printf("state:    %s\n", st.State)
	if st.License == nil {
		fmt.Println("edition:  community (free) — core Ledger works fully; see PRICING.md for Enterprise")
		if st.Warning != "" {
